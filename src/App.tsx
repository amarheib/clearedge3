    ...
    try {
      ...
      const res = await fetch(`${api}/api/case`, {
        ...
        body: JSON.stringify({
          ...
          attachments: [{ kind: "invoice/json", name: "invoice.json", data: invoice }]
        })
      });
        ...
        {open && !caseId && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="md:col-span-3 flex items-center gap-3">
              ...
              <a
                href={`https://wa.me/972527086377?text=${encodeURIComponent("שלום! רוצה לפתוח תיק לבעיה שמצאתי בדוח.")}`}
                target="_blank" rel="noopener noreferrer"
                className="px-4 py-2 rounded-xl border border-emerald-600 text-emerald-700 text-sm font-semibold hover:bg-emerald-50"
              >
                דברו איתי בוואטסאפ
              </a>
            </div>
